...
struct line_type{int start, end, color;};
...
void Bs_rotatef_shp(float ax,float ay,float az){
 ...
 xmat[1][0] = 0; xmat[1][1] = cos((ax*3.14159)/180);xmat[1][2] = sin((ax*3.14159)/180);xmat[1][3] = 0;
 xmat[2][0] = 0; xmat[2][1] = -sin((ax*3.14159)/180);xmat[2][2] = cos((ax*3.14159)/180);xmat[2][3] = 0;
 xmat[3][0] = 0; xmat[3][1] = 0; xmat[3][2] = 0; xmat[3][3] = 1;
 Bs_multiply_matrices(mat1,xmat,matrix);
 // y rotation
 ymat[0][0] = cos((ay*3.14159)/180); ymat[0][1] = 0; ymat[0][2] = -sin((ay*3.14159)/180); ymat[0][3] = 0;
 ymat[1][0] =       0; ymat[1][1] = 1; ymat[1][2] =        0; ymat[1][3] = 0;
 ymat[2][0] = sin((ay*3.14159)/180); ymat[2][1] = 0; ymat[2][2] =  cos((ay*3.14159)/180); ymat[2][3] = 0;
 ymat[3][0] =       0; ymat[3][1] = 0; ymat[3][2] =        0; ymat[3][3] = 1;
 Bs_multiply_matrices(mat2,ymat,mat1);
 // z rotation
 zmat[0][0] = cos((az)*3.14159)/180; zmat[0][1] = sin((az*3.14159)/180); zmat[0][2] = 0; zmat[0][3] = 0;
 zmat[1][0] =-sin((az*3.14159)/180); zmat[1][1] = cos((az*3.14159)/180); zmat[1][2] = 0; zmat[1][3] = 0;
...
int dif = 0, def = 0;
 ...
 dif = 0;
 def = 0;
  if((shape.points[shape.line[i].start].sx+offsetX) == (shape.points[shape.line[i].end].sx+offsetX)){
  dif = 1;
  }
  if((shape.points[shape.line[i].start].sy+offsetY) == (shape.points[shape.line[i].end].sy+offsetY)){
  def = 1;
  }
  display.Bs_draw_line(roundf(shape.points[shape.line[i].start].sx+offsetX+dif),
	   roundf(shape.points[shape.line[i].start].sy+offsetY),
	   roundf(shape.points[shape.line[i].end].sx+offsetX),
	   roundf(shape.points[shape.line[i].end].sy+offsetY+def),
	   shape.line[i].color);
 }
}

void Bs3d_pdraw_3dshp(shapes3d_type shape, Bs_cmd_graph display, int offsetX, int offsetY){
  for(int i = 0; i < shape.vertN; i++){
     display.Bs_put_Pixel(roundf(shape.points[i].sx+offsetX),roundf(shape.points[i].sy+offsetY),shape.color);
  }
}
